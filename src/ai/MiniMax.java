...
public class MiniMax implements AI {
    ...
    private List<int[]> generateMoves() {
        ...
        if (hasWon(mySeed) || hasWon(mySeed * -1)) {
    ...
    private int evaluateLine(int row1, int col1, int row2, int col2, int row3, int col3) {
        ...
        if (board[row1][col1] == mySeed) {
            score = 1;
        } else if (board[row1][col1] == mySeed * -1) {
            score = -1;
        }
        ...
        } else if (board[row2][col2] == mySeed * -1) {
        ...
        } else if (board[row3][col3] == mySeed * -1) {
        ...
        for (int row = 0; row < board.length; ++row) {
            for (int col = 0; col < board.length; ++col) {
                if (board[row][col] == thePlayer) {
                    pattern |= (1 << (row * board.length + col));
                }
            }
        }
